 ...
 */
const constructSettings = exports.constructSettings = (settings = {}, baseModel,
    filters, orders, paging, generalFilterValue, fieldsInGeneralFilter) => {
    ...
    if (generalFilterValue) {
        constructGeneralFilter(settings, baseModel, generalFilterValue, fieldsInGeneralFilter);
    }
 ...
 */
const addPathToSettings = exports.addPathToSettings = (path, settings, baseModel, required = false) => {
    const splitted = path.split(".");
 ...
 */
const constructGeneralFilter = exports.constructGeneralFilter = (settings, baseModel, value, pathsToCompare) => {
    if (!value) {
        return;
    }
    ...
    if (!pathsToCompare) {
        ...
        pathsToCompare = Object.keys(baseModel.rawAttributes).filter(x => pathsExcluded.indexOf(x) < 0);
    }
    ...
    for (let path of pathsToCompare) {
        ...
        const condition = Sequelize.where(Sequelize.cast(Sequelize.col(getAliasedPath(path, baseModel)), 'VARCHAR'), { $iLike: `%${value}%` });
        or.push(condition);
    }
 ...
 * manualmente.
 */
const addFiltersToSettings = exports.addFiltersToSettings = (settings, baseModel, filters) => {
 ...
 * ]
 */
const addOrderToSettings = exports.addOrderToSettings = (settings, baseModel, orders) => {
 ...
 * }
 */
const addPagingToSettings = exports.addPagingToSettings = (settings, baseModel, paging) => {
